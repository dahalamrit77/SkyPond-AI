...
function DEACard() {
  ...
  const r = 33, circ = 2 * Math.PI * r, score = 94;
  return (
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>
        ...
        <div style={{ position:"relative", width:68, height:68 }}>
          <svg width={68} height={68} style={{ transform:"rotate(-90deg)" }}>
            <circle cx={34} cy={34} r={r} fill="none" stroke={`${C.p}15`} strokeWidth={6} />
            <circle cx={34} cy={34} r={r} fill="none" stroke={C.p} strokeWidth={6}
              strokeDasharray={circ} strokeDashoffset={on ? circ-(score/100)*circ : circ}
  ...
  return (
    <div ref={ref} style={{ background:C.surface, border:`1.5px solid ${C.border}`,
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
        <div>
          <div style={{ fontSize:10, fontWeight:700, textTransform:"uppercase", letterSpacing:"0.08em", color:C.violet, marginBottom:3 }}>CS Inventory</div>
          ...
          <div style={{ display:"flex", justifyContent:"space-between", marginBottom:4 }}>
            <div style={{ display:"flex", gap:6, alignItems:"center" }}>
              <span style={{ fontSize:9, padding:"1px 5px", borderRadius:3,
                background:`${C.violet}15`, color:C.violet, fontWeight:700 }}>Sch {d.sch}</span>
...
function AnalyticsCard() {
  ...
  const kpis = [
    ...
    { l:"Avg Fill Time",      v:"4.2m",  d:"-0.8m", c:C.violet },
  ];
  const dispensing = [420,455,438,490,462,510,487];
  const maxD = 510;
  const w = 160, h = 40;
  const pts = dispensing.map((v,i) => `${(i/(dispensing.length-1))*w},${h-(v/maxD)*h}`).join(" ");
  const facilities = [
      ...
      <div style={{ padding:"12px 12px", borderRadius:9, background:C.bg, border:`1px solid ${C.border}`, marginBottom:14 }}>
        ...
        {on && (
          <svg width={w} height={h} style={{ display:"block" }}>
            <polyline fill="none" stroke={C.p} strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round" points={pts} />
            {dispensing.map((v,i) => (
              <circle key={i} cx={(i/(dispensing.length-1))*w} cy={h-(v/maxD)*h}
                r={3} fill={C.p} opacity={i===dispensing.length-1?1:0.4} />
            ))}
          </svg>
        )}
      </div>
...
function NavDropdown({ label, items, onAnchor }) {
    ...
    <div ref={ref} style={{ position:"relative" }}
      ...
      <button
        style={{ display:"flex", alignItems:"center", gap:5, background:"none", border:"none",
          color:open ? C.p : C.body, fontSize:13.5, fontWeight:600, cursor:"pointer",
      ...
      {open && (
        <div style={{ position:"absolute", top:"calc(100% + 12px)", left:"50%",
            ...
            {items.map((item, i) => (
              <a key={i} href={item.slug}
                ...
                <div>
                  ...
                  <div style={{ color:C.muted, fontSize:11.5, marginTop:2 }}>{item.desc}</div>
                </div>
              </a>
            ))}
          </div>
          <div style={{ borderTop:`1px solid ${C.border}`, marginTop:8, paddingTop:8,
            ...
            <a href={label === "Services" ? "/services" : "/products"}
  ...
  return (
    <nav style={{ position:"fixed", top:0, left:0, right:0, zIndex:100, height:64, padding:"0 5vw",
      ...
      <div style={{ display:"flex", alignItems:"center", gap:26 }}>
        <NavDropdown label="Services" items={SERVICES_NAV} />
        <NavDropdown label="Products" items={PRODUCTS_NAV} />
        <a href="/industries" style={{ color:C.body, fontSize:13.5, fontWeight:600,
          textDecoration:"none", transition:"color 0.15s" }}
          onMouseEnter={e => e.currentTarget.style.color=C.p}
          onMouseLeave={e => e.currentTarget.style.color=C.body}>Industries</a>
        {["About","Contact"].map(l => (
          <button key={l} onClick={() => go(l.toLowerCase())}
            style={{ background:"none", border:"none", color:C.body, fontSize:13.5, fontWeight:600,
              cursor:"pointer", fontFamily:"inherit", transition:"color 0.15s" }}
            onMouseEnter={e => e.currentTarget.style.color=C.p}
            onMouseLeave={e => e.currentTarget.style.color=C.body}>{l}</button>
        ))}
      </div>
      ...
      <div style={{ maxWidth:1100, margin:"0 auto" }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-end",
          ...
          <div>
            ...
            <H size="h2" style={{ marginBottom:10, maxWidth:480 }}>LTC-Focused. Not LTC-Only.</H>
...
function Testimonials() {
  ...
  return (
    <section id="about" style={{ padding:"92px 5vw", background:C.bg }}>
    ...
    <footer style={{ background:C.surface, borderTop:`1px solid ${C.border}`, padding:"44px 5vw 22px" }}>
      <div style={{ maxWidth:1100, margin:"0 auto" }}>
          ...
          <div>
            <div style={{ display:"flex", alignItems:"center", gap:9, marginBottom:12 }}>
              <div style={{ width:30, height:30, borderRadius:7,
                ...
                display:"flex", alignItems:"center", justifyContent:"center",
                fontWeight:900, fontSize:14, color:"#fff" }}>S</div>
              <span style={{ color:C.head, fontWeight:800, fontSize:16, fontFamily:"'DM Sans',sans-serif" }}>
                SkypondTech<span style={{ color:C.p }}>.ai</span>
              </span>
            </div>
            <p style={{ color:C.muted, fontSize:13.5, lineHeight:1.7, maxWidth:240, marginBottom:16 }}>
              The complete LTC pharmacy technology platform. DEA compliance, AI automation, analytics, PointClickCare, and more.
            </p>
            {[["📍 Lafayette, CO","#"],["✉ info@skypondtech.com","mailto:info@skypondtech.com"],["☎ (720) 724-6828","tel:+17207246828"]].map(([t,h]) => (
              <a key={t} href={h} style={{ display:"block", color:C.muted, fontSize:12.5, textDecoration:"none", marginBottom:4 }}>{t}</a>
            ))}
          </div>
          {[
            ["Products",[["DEA Lookup Tool","#"],["DEA Compliance Reporting","#"],["CS Inventory","#"],["LTC Analytics","#"],["PointClickCare Feed","#"],["Document Automation","#"]]],
            ["Services",[["LTC Pharmacy IT","#"],["AI Automation","#"],["Data Analytics","#"],["Custom Dev","#"],["Microsoft Cloud","#"]]],
            ["Company",[["Our Values","https://skypondtech.com/ourvalues/"],["Case Studies","https://skypondtech.com/resources/case-studies/"],["Blog","https://skypondtech.com/resources/blogs/"],["FAQs","https://skypondtech.com/resources/faqs/"],["skypondtech.com","https://skypondtech.com"]]]
          ].map(([title, links]) => (
            <div key={title}>
              <div style={{ color:C.head, fontWeight:700, fontSize:11.5, letterSpacing:"0.06em",
                textTransform:"uppercase", marginBottom:13 }}>{title}</div>
              {links.map(([lbl, href]) => (
                <a key={lbl} href={href} style={{ display:"block", color:C.muted, fontSize:13.5,
        ...
        <div style={{ borderTop:`1px solid ${C.border}`, paddingTop:18, display:"flex",
          ...
          <span style={{ color:C.muted, fontSize:12.5 }}>© 2025 Skypond Tech Pvt. Ltd. · All Rights Reserved</span>
          <div style={{ display:"flex", gap:18 }}>
            {[["LinkedIn","https://www.linkedin.com/company/skypond-tech-llc/"],["Instagram","https://www.instagram.com/skypondtech/"]].map(([l,h]) => (
              <a key={l} href={h} style={{ color:C.muted, fontSize:12.5, textDecoration:"none" }}
                onMouseEnter={e => e.currentTarget.style.color=C.p}
    ...
    <>
      <style>{`
        ...
        *{margin:0;padding:0;box-sizing:border-box;}
        ...
        ::-webkit-scrollbar-track{background:#F6F7FD;}
