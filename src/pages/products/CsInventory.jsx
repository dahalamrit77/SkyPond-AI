// product page -> CS Inventory Page (refactored to match service-page patterns)
...
function DiagNode({ icon, title, sub, pills = [], special = false }) {
  return (
    <div style={{
    ...
    <Mockup title="CS Inventory — Perpetual Balance">
      ...
      <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 10, padding: '12px 14px', marginBottom: 10 }}>
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr', gap: 8, fontSize: 10, color: muted, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 8 }}>
          {['Drug', 'On Hand', 'Expected', 'Status'].map((h) => <span key={h}>{h}</span>)}
        </div>
        {rows.map((r, i) => (
          <div key={i} style={{
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', width: '100%', position: 'relative', zIndex: 1 }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1.32fr) minmax(0, 0.82fr)', gap: 56, alignItems: 'center' }}>
          <div style={{ minWidth: 0, animation: 'fadeUp 0.6s ease both' }}>
            <div style={{ marginBottom: 22 }}><Badge c={C.p}><Package size={12} /> Inventory Management</Badge></div>
            <H size="hero" style={{ marginBottom: 22, textWrap: 'balance' }}>
              Real-time{' '}
              <span style={{ color: C.p }}>controlled substance</span>
              {' '}inventory built for LTC volume
            </H>
            <P style={{ marginBottom: 36, fontSize: '1rem', maxWidth: '100%' }}>
              Track every dispense, receive, return, and waste transaction against a perpetual running balance. Surface discrepancies instantly — before they become compliance events that demand your attention during an inspection.
            </P>
...
export default function CsInventory() {
  return (
    <>
      <Navbar />
      <Breadcrumb items={[{ label: 'Home', href: '/' }, { label: 'Products', href: '/products' }, { label: 'CS Inventory Management', href: null }]} />
      <main>
        <Hero />
        <Features />
        <HowItWorks />
