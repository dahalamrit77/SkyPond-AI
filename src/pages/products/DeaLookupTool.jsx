// product page -> DEA Lookup Tool Page (refactored to match service-page patterns)
...
/* ── Primitives (match service-page pattern exactly) ── */
...
function DiagNode({ icon, title, sub, pills, special }) {
  return (
    <div style={{
    ...
    <Mockup title="DEA Lookup — Verification Result">
      ...
      <div style={{ marginBottom: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          {['Schedule II', 'Schedule III', 'Schedule IV', 'Schedule V'].map((s) => (
            <span key={s} style={{
...
export default function DeaLookupTool() {
  return (
    <>
      <Navbar />
      <Breadcrumb items={[{ label: 'Home', href: '/' }, { label: 'Products', href: '/products' }, { label: 'DEA Lookup Tool', href: null }]} />
      <main>
        <Hero />
        <Features />
        <HowItWorks />
