...
function Badge({ c = C.p2, children }: { c?: string; children: React.ReactNode }) {
  return (
    <span style={{ display:'inline-flex', alignItems:'center', gap:5, padding:'4px 13px',
      ...
      fontFamily:"'Akshar', sans-serif" }}>{children}</span>
  )
}
function H({ size='h2', style={}, color, children }: { size?:string; style?:React.CSSProperties; color?:string; children:React.ReactNode }) {
  const sizes: Record<string,string> = { hero:'clamp(2.2rem,4.5vw,3.6rem)', h2:'clamp(1.7rem,2.5vw,2.3rem)', h3:'1.15rem' }
  const Tag = size==='hero'||size==='h1' ? 'h1' : size==='h3' ? 'h3' : 'h2'
  return <Tag style={{ fontSize:sizes[size]||sizes.h2, fontWeight:size==='h3'?500:700, color:color||C.head,
    letterSpacing:'-0.026em', lineHeight:1.12, fontFamily:"'Akshar', sans-serif", ...style }}>{children}</Tag>
}
function P({ style={}, children }: { style?:React.CSSProperties; children:React.ReactNode }) {
  return <p style={{ fontSize:'clamp(0.95rem,1.05vw,1.02rem)', color:C.body, lineHeight:1.78,
...
function PrimaryVerticals() {
  const verticals = [
    {
      tag:'Primary Focus', tagColor:C.p, icon:'💊', color:C.p,
      title:'LTC Pharmacy', subtitle:'Our deepest domain — built from the inside out',
      ...
      cta:'View LTC Pharmacy Services', ctaHref:'/services',
    },
    {
      tag:'Near-Primary', tagColor:C.p, icon:'🏥', color:C.p,
      title:'LTC Facilities', subtitle:'SNF, ALF, Memory Care & Behavioral Health',
      ...
      cta:'Talk to Our Team', ctaHref:'/schedule-demo',
    },
  ]
  return (
    <section style={{ padding:'88px 5vw', background:C.surface }}>
      <div style={{ maxWidth:1100, margin:'0 auto' }}>
        <div style={{ textAlign:'center', marginBottom:52 }}>
          ...
          <P style={{ maxWidth:520, margin:'0 auto' }}>Two verticals where our team has real, hands-on domain knowledge — not just technology experience.</P>
        </div>
          ...
          {verticals.map((v, i) => (
            <div key={i} style={{ borderRadius:20, border:`2px solid ${v.color}30`,
              ...
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:20 }}>
                <div style={{ display:'flex', alignItems:'center', gap:12 }}>
                  <div style={{ width:48, height:48, borderRadius:12, background:`${v.color}14`,
                    border:`1.5px solid ${v.color}35`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:22 }}>{v.icon}</div>
                  <div>
                    <div style={{ fontSize:10.5, fontWeight:700, color:v.tagColor, textTransform:'uppercase', letterSpacing:'0.07em' }}>{v.tag}</div>
                    <h3 style={{ fontSize:20, marginTop:2, fontWeight:500, color:C.head, fontFamily:"'Akshar', sans-serif" }}>{v.title}</h3>
                  </div>
                </div>
              </div>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginBottom:24 }}>
                <div>
                  <div style={{ fontSize:10.5, fontWeight:700, color:C.muted, textTransform:'uppercase', letterSpacing:'0.07em', marginBottom:10 }}>What We Do</div>
                ...
                <div>
                  <div style={{ fontSize:10.5, fontWeight:700, color:C.muted, textTransform:'uppercase', letterSpacing:'0.07em', marginBottom:10 }}>Systems We Know</div>
                  <div style={{ display:'flex', flexWrap:'wrap', gap:6 }}>
                    {v.systems.map((s, j) => (
                      <span key={j} style={{ padding:'4px 10px', borderRadius:6, fontSize:12,
                        background:`${v.color}10`, border:`1px solid ${v.color}25`, color:v.color, fontWeight:600 }}>{s}</span>
                    ))}
                  </div>
                </div>
              </div>
              <Button to={v.ctaHref} variant="primary" size="sm" style={{ alignSelf:'flex-start' }}>{v.cta} →</Button>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ maxWidth:1100, margin:'0 auto' }}>
        ...
        <div style={{ padding:'18px 24px', borderRadius:12, marginBottom:44,
          ...
          <P style={{ fontSize:'0.93rem', marginBottom:0 }}>
            Two of our six services — <strong style={{ color:C.p }}>Data Analytics & Power BI</strong> and <strong style={{ color:C.p }}>Custom Development</strong> — apply cleanly outside the healthcare space.
          </P>
        </div>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:20 }}>
          {industries.map((ind, i) => (
            <div key={i} style={{ padding:'28px 26px', borderRadius:18, background:C.surface,
              ...
              onMouseEnter={e => { const el=e.currentTarget; el.style.borderColor=ind.color+'50'; el.style.transform='translateY(-4px)'; el.style.boxShadow=`0 16px 36px ${ind.color}14`; }}
              ...
              <div style={{ width:44, height:44, borderRadius:11, background:`${ind.color}12`,
                border:`1.5px solid ${ind.color}30`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:20, marginBottom:16 }}>{ind.icon}</div>
              <h3 style={{ fontSize:17, marginBottom:10, fontWeight:500, color:C.head, fontFamily:"'Akshar', sans-serif" }}>{ind.title}</h3>
        ...
        <div style={{ textAlign:'center', marginTop:40 }}>
          <Button to="/services/data-analytics" variant="primary" size="md" style={{ marginRight:12 }}>Data Analytics Service →</Button>
          <Button to="/services/custom-development" variant="secondary" size="md">Custom Development Service →</Button>
        </div>
      </div>
    </section>
  )
    ...
    <section style={{ padding:'88px 5vw', background:C.surface }}>
      <div style={{ maxWidth:860, margin:'0 auto' }}>
        <div style={{ textAlign:'center', marginBottom:48 }}>
          ...
          <P style={{ maxWidth:520, margin:'0 auto' }}>We're not the right partner for everyone — and we'd rather say that upfront than take on a project we can't do well.</P>
        </div>
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
          <div style={{ padding:'28px 26px', borderRadius:16, background:`${C.green}07`, border:`2px solid ${C.green}25` }}>
            <div style={{ fontSize:11, fontWeight:700, color:C.green, textTransform:'uppercase', letterSpacing:'0.08em', marginBottom:18 }}>✓ Strong Fit</div>
            {['LTC pharmacy or long-term care facility — any scope','Organization needing serious analytics or BI work','Custom web application with complex business logic','System integration across multiple data sources','Team that wants a long-term technology partner','Project where domain knowledge actually matters'].map((item,i,a)=>(
              <div key={i} style={{ display:'flex', gap:10, alignItems:'flex-start', padding:'9px 0', borderBottom:i<a.length-1?`1px solid ${C.green}15`:'none' }}>
          ...
          <div style={{ padding:'28px 26px', borderRadius:16, background:`${C.muted}07`, border:`2px solid ${C.border}` }}>
            <div style={{ fontSize:11, fontWeight:700, color:C.muted, textTransform:'uppercase', letterSpacing:'0.08em', marginBottom:18 }}>✗ Likely Not a Fit</div>
            {['General IT support or helpdesk services','Simple website or marketing site builds','Industries outside our experience without analytics/dev needs','Projects where lowest price is the primary decision factor','One-time deliverable with no ongoing relationship','Timelines that require a large team on day one'].map((item,i,a)=>(
              <div key={i} style={{ display:'flex', gap:10, alignItems:'flex-start', padding:'9px 0', borderBottom:i<a.length-1?`1px solid ${C.border}`:'none' }}>
...
function CTA() {
  return (
    <section style={{ padding:'80px 5vw', background:C.surface, borderTop:`1px solid ${C.border}`, position:'relative' }}>
      <div style={{ maxWidth:620, margin:'0 auto', textAlign:'center', position:'relative', zIndex:1 }}>
        <H size="h2" style={{ color:C.head, marginBottom:14, fontWeight:700 }}>Not Sure If There's a Fit? Just Ask.</H>
