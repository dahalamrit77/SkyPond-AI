...
/* ─── Page-local primitives (preserves exact font sizes from original) ─── */
...
function H({ size='h2', style={}, color, children }: { size?:string; style?:React.CSSProperties; color?:string; children:React.ReactNode }) {
  const sizes: Record<string,string> = { hero:'clamp(2.4rem,5vw,3.8rem)', h2:'clamp(1.8rem,2.7vw,2.4rem)', h3:'1.15rem' }
...
function P({ style={}, children }: { style?:React.CSSProperties; children:React.ReactNode }) {
  return <p style={{ fontSize:'clamp(0.96rem,1.1vw,1.04rem)', color:C.body, lineHeight:1.78,
...
function Mission() {
  const pillars = [
    ...
    { icon:<LinkIcon size={22} />, color:C.amber, title:'Fluent in the Systems You Already Use',
      desc:"We work directly with the pharmacy management systems and EHRs LTC pharmacies run on — QS/1 (NRx), FrameworkLTC, MatrixCare, American HealthTech (AHT), and PointClickCare. No ramp-up time learning your stack." },
    ...
    <section style={{ padding:'88px 5vw', background:C.alt }}>
      <div style={{ maxWidth:1060, margin:'0 auto' }}>
        <div style={{ textAlign:'center', marginBottom:52 }}>
          ...
          <H size="h2" style={{ marginBottom:14 }}>What We're Here to Do</H>
          <P style={{ maxWidth:520, margin:'0 auto' }}>Three principles that shape every project we take on and every relationship we build.</P>
        </div>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(2,1fr)', gap:20 }}>
          {pillars.map((p, i) => (
            <div key={i} style={{ padding:'32px 28px', borderRadius:18, background:C.surface,
              ...
              onMouseEnter={e => { const el = e.currentTarget; el.style.borderColor=p.color+'50'; el.style.boxShadow=`0 16px 40px ${p.color}14`; el.style.transform='translateY(-4px)'; }}
              ...
              <h3 style={{ marginBottom:10, fontSize:16, color:C.head, fontWeight:500,
    ...
    <section style={{ padding:'88px 5vw', background:C.surface }}>
      <div style={{ maxWidth:900, margin:'0 auto' }}>
        <div style={{ textAlign:'center', marginBottom:48 }}>
          ...
          <P style={{ maxWidth:480, margin:'0 auto' }}>We'd rather set the right expectations upfront than be everything to everyone.</P>
        </div>
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
          <div style={{ padding:'28px 26px', borderRadius:16, background:`${C.green}08`, border:`2px solid ${C.green}30` }}>
            <div style={{ fontSize:11, fontWeight:700, color:C.green, textTransform:'uppercase', letterSpacing:'0.08em', marginBottom:18 }}>✓ What We Are</div>
            {['A focused team with deep LTC pharmacy IT expertise','Specialists in PointClickCare integrations and API connections','Builders of custom tools for pharmacy-specific workflows','Honest advisors who tell you what\'s realistic','A long-term partner, not a one-time vendor','Colorado-based, serving LTC pharmacies nationwide'].map((item, i) => (
              <div key={i} style={{ display:'flex', gap:10, alignItems:'flex-start', padding:'9px 0', borderBottom: i<5 ? `1px solid ${C.green}18` : 'none' }}>
          ...
          <div style={{ padding:'28px 26px', borderRadius:16, background:`${C.muted}08`, border:`2px solid ${C.border}` }}>
            <div style={{ fontSize:11, fontWeight:700, color:C.muted, textTransform:'uppercase', letterSpacing:'0.08em', marginBottom:18 }}>✗ What We're Not</div>
            {['A general healthcare IT firm that also serves LTC','A large agency with hundreds of consultants','A company that will say yes to any project in any industry','A managed services provider for general IT support','A firm that competes on lowest price over best fit','A vendor that disappears after go-live'].map((item, i) => (
              <div key={i} style={{ display:'flex', gap:10, alignItems:'flex-start', padding:'9px 0', borderBottom: i<5 ? `1px solid ${C.border}` : 'none' }}>
...
function CTA() {
  return (
    <section style={{ padding:'80px 5vw', background:C.bg, borderTop:`1px solid ${C.border}`, position:'relative' }}>
      <div style={{ maxWidth:640, margin:'0 auto', textAlign:'center', position:'relative', zIndex:1 }}>
        <H size="h2" style={{ color:C.head, marginBottom:14, fontWeight:700 }}>
