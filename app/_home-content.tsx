...
import {
  ...
  Inbox, Activity, Zap, Building2, Users,
} from 'lucide-react'
...
/**
 * Page-local H — preserves exact Home font sizes.
 * Fixes h-tag semantics: hero→h1, h2→h2, h3→h3 (original always rendered h2).
 */
function H({ size = 'h2', style = {}, color, children }: {
  ...
  const Tag = size === 'hero' || size === 'h1' ? 'h1' : size === 'h3' ? 'h3' : 'h2'
  return (
    <Tag style={{
    ...
    <div style={{
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr' }}>
        ...
        <div style={{ padding: '20px 20px', borderRight: '1px solid rgba(255,255,255,0.06)' }}>
          <div style={{ fontSize: 10.5, fontWeight: 700, color: 'rgba(255,255,255,0.3)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 14 }}>AI Pipeline</div>
          {pipeline.map((s, i) => {
            ...
            return (
              <div key={i} style={{ display: 'flex', gap: 11, marginBottom: 9, alignItems: 'flex-start' }}>
                ...
                <div>
                  ...
                  {isActive && (
                    <div style={{ fontSize: 10.5, color: C.accent, marginTop: 2, animation: 'fadeUp 0.3s ease both' }}>{s.sub}</div>
                  )}
                </div>
              </div>
            )
        ...
        <div style={{ padding: '20px 18px' }}>
          <div style={{ fontSize: 10.5, fontWeight: 700, color: 'rgba(255,255,255,0.3)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 14 }}>Order Queue</div>
          {orders.map((o, i) => (
            <div key={i} style={{
              ...
              <div>
                <div style={{ fontSize: 12, color: o.active ? '#fff' : 'rgba(255,255,255,0.38)', fontWeight: o.active ? 600 : 400 }}>{o.drug}</div>
          ...
          <div style={{ marginTop: 14, display: 'flex', flexDirection: 'column', gap: 7 }}>
            <div style={{ padding: '8px 11px', borderRadius: 8, background: `${C.green}12`, border: `1px solid ${C.green}28` }}>
            ...
            <div style={{ padding: '8px 11px', borderRadius: 8, background: `${C.p}12`, border: `1px solid ${C.p}28` }}>
...
function DEACard() {
  ...
  return (
    <div ref={ref} style={{ background: C.surface, border: `1.5px solid ${C.border}`, borderRadius: 18, padding: '22px 20px', boxShadow: `0 4px 20px ${C.p}08` }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        ...
        <div style={{ position: 'relative', width: 68, height: 68, borderRadius: '50%', border: `2px solid ${C.p}30`, background: `${C.p}08` }}>
          <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <span style={{ fontSize: 17, fontWeight: 700, color: C.p, lineHeight: 1, fontFamily: "'Akshar', sans-serif" }}>{on ? 94 : 0}</span>
      ...
      <div style={{ marginTop: 12, padding: '8px 11px', borderRadius: 8, background: `${C.green}10`, border: `1px solid ${C.green}28`, display: 'flex', alignItems: 'center', gap: 7 }}>
        <span>✅</span>
...
function CSCard() {
  ...
  return (
    <div ref={ref} style={{ background: C.surface, border: `1.5px solid ${C.border}`, borderRadius: 18, padding: '22px 20px', boxShadow: `0 4px 20px ${C.violet}08` }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
        ...
        <div style={{ padding: '3px 9px', borderRadius: 5, background: `${C.green}15`, border: `1px solid ${C.green}30`, fontSize: 10, fontWeight: 700, color: C.green }}>🟢 Live</div>
      </div>
      {drugs.map((d, i) => (
        <div key={i} style={{ marginBottom: 11, opacity: on ? 1 : 0, transform: on ? 'none' : 'translateX(-6px)', transition: `opacity 0.4s ${i * 0.1}s,transform 0.4s ${i * 0.1}s` }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
            <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
              <span style={{ fontSize: 9, padding: '1px 5px', borderRadius: 3, background: `${C.violet}15`, color: C.p, fontWeight: 700 }}>Sch {d.sch}</span>
...
function AnalyticsCard() {
  ...
  return (
    <div ref={ref} style={{ background: C.surface, border: `1.5px solid ${C.border}`, borderRadius: 18, padding: '22px 20px', boxShadow: `0 4px 20px ${C.accent}08` }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 14 }}>
        {kpis.map((k, i) => (
          <div key={i} style={{ padding: '10px 10px', borderRadius: 9, background: C.bg, border: `1px solid ${C.border}`, opacity: on ? 1 : 0, transform: on ? 'none' : 'translateY(6px)', transition: `opacity 0.4s ${i * 0.1}s,transform 0.4s ${i * 0.1}s` }}>
      ...
      <div>
        ...
        {facilities.map((f, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 7, opacity: on ? 1 : 0, transition: `opacity 0.4s ${0.4 + i * 0.08}s` }}>
            ...
            <div style={{ width: 80, height: 5, borderRadius: 3, background: C.alt, overflow: 'hidden', flexShrink: 0 }}>
              <div style={{ height: '100%', borderRadius: 3, background: f.c, width: on ? `${f.pct}%` : '0%', transition: `width 0.8s ${0.4 + i * 0.08}s ease-out` }} />
            </div>
...
function Hero() {
  ...
  return (
    <section style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', padding: '120px 5vw 80px', position: 'relative', overflow: 'hidden', background: C.surface }}>
      <div style={{ maxWidth: 1160, margin: '0 auto', width: '100%', position: 'relative', zIndex: 1 }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 64, alignItems: 'center' }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 28, animation: 'fadeUp 0.6s 0.2s ease both' }}>
              {[
                ...
                <div key={txt} style={{ display: 'inline-flex', alignItems: 'center', gap: 10, padding: '10px 14px', borderRadius: 9, background: `${C.p}0A`, border: `1px solid ${C.p}20` }}>
            ...
            {/* Stats */}
            <div style={{ display: 'flex', gap: 0, marginTop: 48, borderTop: `1px solid ${C.border}`, paddingTop: 30, animation: 'fadeUp 0.6s 0.4s ease both' }}>
              {[['50+', 'Projects'], ['100%', 'Satisfaction'], ['6', 'Products'], ['LTC', 'Only Focus']].map(([v, l], i, a) => (
                <div key={i} style={{ flex: 1, paddingRight: 18, borderRight: i < a.length - 1 ? `1px solid ${C.border}` : 'none', marginRight: i < a.length - 1 ? 18 : 0 }}>
                  <div style={{ fontSize: 'clamp(1.4rem,2.2vw,1.9rem)', fontWeight: 700, color: C.p, letterSpacing: '-0.03em', fontFamily: "'Akshar', sans-serif" }}>{v}</div>
                  <div style={{ color: C.muted, fontSize: 11.5, marginTop: 2, fontWeight: 300, fontFamily: "'Gotham', 'Helvetica Neue', Arial, sans-serif" }}>{l}</div>
                </div>
              ))}
            </div>
          </div>
  ...
  return (
    <div style={{ background: C.dark, padding: '15px 0', overflow: 'hidden', whiteSpace: 'nowrap' }}>
      <div style={{ display: 'inline-flex', gap: 52, animation: 'ticker 32s linear infinite' }}>
        {[...pills, ...pills].map((p, i) => (
          <span key={i} style={{ color: 'rgba(255,255,255,0.38)', fontSize: 11.5, fontWeight: 700, letterSpacing: '0.07em', textTransform: 'uppercase' }}>✦ {p}</span>
        ))}
      </div>
    </div>
  )
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.15fr', gap: 24 }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
            {PRODUCTS.map((pr, i) => (
              <button key={i} onClick={() => setActive(i)} style={{
                display: 'flex', alignItems: 'center', gap: 13, padding: '12px 15px', borderRadius: 11,
                textAlign: 'left', background: active === i ? `${pr.color}0C` : 'transparent',
                border: `1.5px solid ${active === i ? C.p2 : C.border}`,
                cursor: 'pointer', transition: 'all 0.18s', fontFamily: "'Akshar', sans-serif", fontWeight: 500,
              }}>
                <span style={{ fontSize: 17, width: 37, height: 37, flexShrink: 0, borderRadius: 9, background: active === i ? `${pr.color}18` : C.alt, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{pr.icon}</span>
                <div style={{ flex: 1 }}>
                  <div style={{ color: C.head, fontWeight: 700, fontSize: 13.5 }}>{pr.title}</div>
                  <div style={{ color: pr.color, fontSize: 10.5, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', marginTop: 2 }}>{pr.tag}</div>
                </div>
          ...
          <Card ac={p.color} hover={false} style={{ padding: '28px 26px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 13, marginBottom: 18 }}>
              <span style={{ fontSize: 28, width: 52, height: 52, background: `${p.color}14`, border: `1px solid ${p.color}28`, borderRadius: 13, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{p.icon}</span>
              <div>
                <span style={{ fontSize: 10.5, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: p.color }}>{p.tag}</span>
                <h3 style={{ color: C.head, fontWeight: 500, fontSize: 18, marginTop: 2, fontFamily: "'Akshar', sans-serif" }}>{p.title}</h3>
              </div>
            </div>
            <P style={{ marginBottom: 18, lineHeight: 1.72 }}>{p.desc}</P>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '7px 14px', borderRadius: 8, background: `${p.color}10`, border: `1px solid ${p.color}25`, color: p.color, fontWeight: 700, fontSize: 12.5, marginBottom: 22 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(280px,1fr))', gap: 18 }}>
          {SERVICES.map((s, i) => (
            <Card key={i} ac={s.color} style={{ padding: '24px 22px 26px' }}>
              <div style={{ width: 46, height: 46, borderRadius: 10, marginBottom: 14, background: `${s.color}10`, border: `1px solid ${s.color}25`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 21 }}>{s.icon}</div>
              <h3 style={{ color: C.head, fontWeight: 500, fontSize: 16, marginBottom: 8, fontFamily: "'Akshar', sans-serif" }}>{s.title}</h3>
              <p style={{ color: C.body, fontSize: 13.5, lineHeight: 1.68, fontFamily: "'Gotham', 'Helvetica Neue', Arial, sans-serif", fontWeight: 400 }}>{s.desc}</p>
            </Card>
          ))}
        </div>
      </div>
...
function IndustriesSection() {
  const verticals = [
    { icon: <Pill size={20} />, color: C.p, tag: 'Primary Focus', title: 'LTC Pharmacy',
      desc: 'Our deepest domain. Nearly a decade inside LTC pharmacy operations — dispensing, DEA compliance, PointClickCare, prior auth, and everything in between.', href: '/services' },
    { icon: <Hospital size={20} />, color: C.p, tag: 'Near-Primary', title: 'LTC Facilities',
      desc: 'SNF, ALF, memory care, and behavioral health. Same compliance demands, overlapping systems, same need for technology that understands the care setting.', href: '/industries' },
    { icon: <BarChart3 size={20} />, color: C.amber, tag: 'Analytics & Dev', title: 'Financial & Retail',
      desc: 'Our Data Analytics and Custom Development services travel outside healthcare. We\'ve built analytics platforms and custom applications for financial firms and retail organizations.', href: '/industries' },
  ]
  return (
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 44, flexWrap: 'wrap', gap: 16 }}>
          <div>
            ...
            <H size="h2" style={{ marginBottom: 10, maxWidth: 480 }}>LTC-Focused.<br />Not LTC-Only.</H>
            <P style={{ maxWidth: 460 }}>Our expertise is rooted in long-term care. Our analytics and development capabilities extend beyond it.</P>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 20 }}>
          {verticals.map((v, i) => (
            <a key={i} href={v.href} style={{ textDecoration: 'none' }}>
              <div style={{ padding: '28px 26px', borderRadius: 18, background: C.surface, border: `1.5px solid ${C.border}`, height: '100%', transition: 'all 0.2s', display: 'flex', flexDirection: 'column', gap: 0 }}
                ...
                <div style={{ width: 44, height: 44, borderRadius: 11, background: `${v.color}12`, border: `1.5px solid ${v.color}28`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, marginBottom: 16 }}>{v.icon}</div>
                <div style={{ fontSize: 10.5, fontWeight: 700, color: v.color, textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 6 }}>{v.tag}</div>
                <div style={{ color: C.head, fontWeight: 700, fontSize: 17, marginBottom: 10, fontFamily: "'Akshar', sans-serif" }}>{v.title}</div>
...
function Testimonials() {
  ...
  useEffect(() => {
    sanityClient.fetch<Testimonial[]>(TESTIMONIALS_QUERY)
      ...
      // Silently hide testimonials if Sanity is unreachable or not yet configured
    ...
    <section id="about" style={{ padding: '92px 5vw', background: C.surface }}>
      <div style={{ maxWidth: 820, margin: '0 auto', textAlign: 'center' }}>
          ...
          {data.map((t, i) => (
            <div key={t._id} style={{
              ...
              <Card hover={false} style={{ padding: '32px 36px', textAlign: 'left' }}>
                <div style={{ fontSize: 38, lineHeight: 1, color: C.p, marginBottom: 10, fontFamily: 'Georgia,serif', opacity: 0.5 }}>"</div>
                <p style={{ fontSize: 'clamp(0.94rem,1.15vw,1.04rem)', color: C.body, lineHeight: 1.78, fontStyle: 'italic', marginBottom: 22 }}>{t.text}</p>
                <div style={{ display: 'flex', alignItems: 'center', gap: 13 }}>
                  {t.avatar ? (
                    <Image src={t.avatar} alt={t.a} width={42} height={42} style={{ borderRadius: '50%', objectFit: 'cover' }} />
                  ) : (
                    <div style={{ width: 42, height: 42, borderRadius: '50%', background: `linear-gradient(135deg, ${C.p}, ${C.p}88)`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, color: '#fff', fontSize: 13 }}>{getInitials(t.a)}</div>
                  )}
                  <div>
                    <div style={{ color: C.head, fontWeight: 700, fontSize: 14.5 }}>{t.a}</div>
                    <div style={{ color: C.muted, fontSize: 12.5 }}>{t.role} · {t.company}</div>
                  </div>
                </div>
              </Card>
            </div>
        ...
        <div style={{ display: 'flex', gap: 7, justifyContent: 'center', marginTop: 20 }}>
          {data.map((_, i) => (
            <button key={i} onClick={() => setActive(i)} style={{ width: active === i ? 26 : 7, height: 7, borderRadius: 4, background: active === i ? C.p : C.border, border: 'none', cursor: 'pointer', transition: 'all 0.3s' }} />
          ))}
        </div>
      </div>
    </section>
...
function CTABanner() {
  return (
    <section style={{ padding: '80px 5vw', background: C.p, position: 'relative', overflow: 'hidden' }}>
      <div style={{ position: 'absolute', top: -140, right: -80, width: 520, height: 520, borderRadius: '50%', background: 'radial-gradient(circle at 70% 50%, rgba(255,255,255,0.04) 0%, transparent 60%)', pointerEvents: 'none' }} />
      <div style={{ position: 'absolute', bottom: -70, left: 0, width: 380, height: 380, borderRadius: '50%', background: 'radial-gradient(circle at 30% 50%, rgba(255,255,255,0.04) 0%, transparent 60%)', pointerEvents: 'none' }} />
      <div style={{ maxWidth: 680, margin: '0 auto', textAlign: 'center', position: 'relative', zIndex: 1 }}>
        <H size="h2" style={{ color: '#FFFFFF', marginBottom: 14 }}>
...
function Contact() {
  ...
  const inp: React.CSSProperties = {
    width: '100%', padding: '11px 13px', borderRadius: 9, background: C.alt,
  ...
  const lbl: React.CSSProperties = {
    color: C.muted, fontSize: 12.5, display: 'block', marginBottom: 5, fontWeight: 500,
    fontFamily: "'Akshar', sans-serif",
  }

  return (
    <section id="contact" style={{ padding: '92px 5vw', background: C.surface, scrollMarginTop: 96 }}>
      <div style={{ maxWidth: 640, margin: '0 auto' }}>
        ...
        {sent ? (
          <Card hover={false} style={{ padding: '48px 36px', textAlign: 'center' }}>
            ...
            <h3 style={{ color: C.head, fontWeight: 500, fontSize: '1.22rem', marginBottom: 10, fontFamily: "'Akshar', sans-serif" }}>We&apos;ll Be in Touch</h3>
          ...
          <Card hover={false} style={{ padding: '34px 30px' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                <div>
                  <label style={lbl}>Pharmacy / Organization</label>
