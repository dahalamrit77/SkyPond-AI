      ...
      {layers.map((layer: any, i: any) => (
        <div key={i} style={{ marginBottom:i < layers.length-1 ? 16 : 0,
          ...
          <div style={{ display:"flex", flexWrap:"wrap", gap:7 }}>
            {layer.items.map((item: any, j: any) => (
...
/* ─── Page export ─────────────────────────────────────────────────────────── */
export function MicrosoftCloudContent() {
  return (
    <>
      <Breadcrumb items={[{ label: 'Home', href: '/' }, { label: 'Services', href: '/services' }, { label: 'Microsoft Cloud' }]} />
      <main>
        <Hero />
        <Problem />
        <Services />
