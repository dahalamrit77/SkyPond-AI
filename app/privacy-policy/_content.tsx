...
function Badge({ c = C.p, children }: { c?: string; children: React.ReactNode }) {
  return (
    <span style={{
...
/**
 * Page-local H — preserves original PrivacyPolicy font sizes exactly.
...
function Body({ style = {}, children }: { style?: React.CSSProperties; children: React.ReactNode }) {
  return (
    <p style={{
...
function Callout({ children, variant = 'info' }: { children: React.ReactNode; variant?: 'info' | 'alert' }) {
...
function SectionCard({ id, num, title, icon, children }: {
  id: string; num: string; title: string; icon: React.ReactNode; children: React.ReactNode
}) {
  return (
    <article id={id} style={{
      scrollMarginTop: 96,
...
function TableOfContents() {
  return (
    <nav aria-label="Privacy policy sections" style={{ position: 'sticky', top: 88, alignSelf: 'start' }}>
      <div style={{
...
function SummaryStrip() {
  const items = [
    { label: 'Effective Date', value: 'October 27, 2025', href: undefined },
    { label: 'Company', value: 'SkyPond Tech Pvt. Ltd.', href: undefined },
