...
/**
 * Dynamically injects the HubSpot meetings script after the container div
 * is in the DOM. Cleans up any previously injected script on HMR to prevent
 * duplicate embeds during development.
 */
...
function WhatToExpect() {
  const steps = [
    ...
    {
      icon: <Search size={15} />,
      title: 'We identify the actual gap',
  ...
  return (
    <div style={{
      ...
      <div style={{
        fontSize: 10.5, fontWeight: 500, color: C.muted, textTransform: 'uppercase',
        letterSpacing: '0.08em', marginBottom: 20, fontFamily: "'Akshar', sans-serif",
      }}>What happens on the call</div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        {steps.map((s, i) => (
          <div key={i} style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
            <div style={{
              width: 34, height: 34, borderRadius: 8, background: C.alt,
              border: `1px solid ${C.border}`, display: 'flex', alignItems: 'center',
              justifyContent: 'center', fontSize: 15, flexShrink: 0,
            }}>{s.icon}</div>
            <div>
              <div style={{
                color: C.head, fontWeight: 500, fontSize: 14, marginBottom: 3,
                fontFamily: "'Akshar', sans-serif",
              }}>{s.title}</div>
              <div style={{
                color: C.body, fontSize: 13, lineHeight: 1.6, fontWeight: 400,
...
export function ScheduleDemoContent() {
  return (
    <>
      <style>{`
        .meetings-iframe-container { background: #fff; }
      ...
      <main style={{ paddingTop: 64 }}>
        ...
        <div style={{
          ...
          <div style={{ position: 'relative', zIndex: 1, animation: 'fadeUp 0.6s ease both' }}>
            <div style={{
              ...
              textTransform: 'uppercase', marginBottom: 16, fontFamily: "'Akshar', sans-serif",
            }}>📅 Free Discovery Call</div>
            <h1 style={{
              fontSize: 'clamp(1.9rem,4vw,2.9rem)', fontWeight: 700, color: '#fff',
              letterSpacing: '-0.03em', lineHeight: 1.12, marginBottom: 14,
              fontFamily: "'Akshar', sans-serif",
            }}>
              {"Let's talk about your"}{' '}
              <span style={{
                background: `linear-gradient(90deg,${C.accent},${C.p2})`,
            ...
            <p style={{
              fontSize: 'clamp(0.96rem,1.1vw,1.05rem)', color: 'rgb(255, 255, 255)',
        ...
        {/* Main grid */}
        <div style={{
          maxWidth: 1060, margin: '0 auto', padding: '56px 5vw 80px',
          ...
          {/* Right panel */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16, animation: 'fadeUp 0.55s 0.2s ease both' }}>
            <WhatToExpect />
                ...
                {[
                  ...
                  <Button key={i} href={item.href} variant="secondary" size="sm"
                    ...
                    <div>
                      ...
                      <div style={{ color: C.muted, fontSize: 12, fontFamily: "'Gotham', 'Helvetica Neue', Arial, sans-serif", fontWeight: 300 }}>{item.sub}</div>
                    </div>
                  </Button>
                ))}
              </div>
        ...
        <Breadcrumb items={[{ label: 'Home', href: '/' }, { label: 'Schedule a Demo', href: '/schedule-demo' }]} />
      </main>
