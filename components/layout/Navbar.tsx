...
import {
  ChevronDown, X, Menu,
  Hospital, Settings, BarChart3, Link as LinkIcon, Laptop, Cloud,
  Search, ClipboardList, Pill, TrendingUp, FileText,
  Users, Star, Shield,
} from 'lucide-react'
...
const SERVICES_NAV = [
  ...
  { icon: <LinkIcon size={18} />,  label: 'PointClickCare Integration',  slug: '/services/pointclickcare-integration', desc: 'Bidirectional EHR data sync' },
...
const PRODUCTS_NAV = [
  ...
  { icon: <LinkIcon size={18} />,      label: 'PointClickCare Data Feed',   slug: '/products/pointclickcare-feed',     desc: 'Live bidirectional PCC sync' },
...
function NavDropdown({ label, items, active, navDark, onAnchor }: NavDropdownProps) {
    ...
    >
      ...
      {open && (
        ...
        >
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 4 }}>
            {items.map((item, i) => (
              ...
              >
                ...
                <div>
                  <div style={{ color: C.head, fontWeight: 500, fontSize: 13.5, fontFamily: "'Akshar', sans-serif" }}>
                    {item.label}
                  </div>
                  <div style={{ color: C.body, fontSize: 11.5, marginTop: 2, fontFamily: "'Gotham', 'Helvetica Neue', Arial, sans-serif", fontWeight: 300 }}>
                    {item.desc}
                  </div>
                </div>
              </Link>
          ...
          <div style={{ borderTop: `1px solid ${C.border}`, marginTop: 8, paddingTop: 8, padding: '10px 12px 6px' }}>
            <Link
...
function AboutDropdown({ active, navDark }: { active: boolean; navDark: boolean }) {
  const [open, setOpen] = useState(false)
  const ref = useRef<HTMLDivElement>(null)
...
function HamburgerIcon({ open, navDark }: { open: boolean; navDark: boolean }) {
  const stroke = navDark ? '#FFFFFF' : C.p
  return open
    ? <X    size={20} color={stroke} aria-hidden />
    : <Menu size={22} color={stroke} aria-hidden />
}

/* ─── Navbar ────────────────────────────────────────────────────────────── */

/**
 * Site navigation — migrated from src/components/Navbar.jsx.
 * Changes vs original:
 *   - import.meta.env.BASE_URL removed → logo paths hardcoded (/navbar-logo-white.png)
 *   - react-router useNavigate → next/navigation useRouter
 *   - react-router useLocation → next/navigation usePathname
 *   - react-router Link → next/link Link
 *   - Hash tracking via window.location + hashchange event
 */
export function Navbar() {
  ...
  /* Track URL hash for "Contact" active state */
  useEffect(() => {
  ...
  /* ── Active-state flags ── */
  const p = pathname ?? '/'
  const activeServices   = p.startsWith('/services')
  const activeProducts   = p.startsWith('/products')
  const activeIndustries = p === '/industries'
  ...
  const deskLinkBase = (isActive: boolean): React.CSSProperties => ({
    fontSize:      13.5,
    fontFamily:    "'Akshar', sans-serif",
    textDecoration: 'none',
    transition:    'color 0.15s, border-color 0.15s',
  ...
  return (
    <>
      ...
      >
        ...
        <Link
          href="/"
          style={{ display: 'flex', alignItems: 'center', textDecoration: 'none', background: 'transparent', lineHeight: 0 }}
          aria-label="SkyPond Tech home"
        >
          <img
            src={navDark ? '/navbar-logo-white.png' : '/navbar-logo-blue.png'}
            alt="SkyPond Tech"
            style={{
              width:          250,
              height:         'auto',
        ...
        {!isMobile && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 26 }}>
            ...
            <Link
              href="/industries"
              style={deskLinkBase(activeIndustries)}
              onMouseEnter={e => { if (!activeIndustries) e.currentTarget.style.color = navDark ? 'rgba(255,255,255,0.95)' : C.p }}
              onMouseLeave={e => { if (!activeIndustries) e.currentTarget.style.color = navDark ? 'rgba(255,255,255,0.78)' : C.body }}
            >
              Industries
            </Link>
            ...
            <button
              ...
              onMouseEnter={e => { if (!activeContact) (e.currentTarget as HTMLElement).style.color = navDark ? 'rgba(255,255,255,0.95)' : C.p }}
              onMouseLeave={e => { if (!activeContact) (e.currentTarget as HTMLElement).style.color = navDark ? 'rgba(255,255,255,0.78)' : C.body }}
            >
              Contact
            </button>
          </div>
        ...
        {!isMobile ? (
          <Button variant={navDark ? 'primaryDark' : 'primary'} size="sm" to="/schedule-demo">
            Schedule a Demo →
          </Button>
        ) : (
          <button
      ...
      {isMobile && mobileOpen && (
        ...
        >
          <div style={{ paddingBottom: 32 }}>
            ...
            >
              <span style={{ color: activeServices ? C.p2 : '#FFFFFF', fontSize: 15, fontFamily: "'Akshar', sans-serif", fontWeight: activeServices ? 600 : 500 }}>
                Services
              </span>
              <ChevronDown size={14} color="rgba(255,255,255,0.7)" style={{ transform: mobileExpanded === 'services' ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s' }} />
            </button>
              ...
              <div style={{ background: 'rgba(255,255,255,0.06)' }}>
                {SERVICES_NAV.map((item, i) => {
                  ...
                  return (
                    ...
                    >
                      <span style={{ fontSize: 20, width: 40, height: 40, borderRadius: 10, flexShrink: 0, background: 'rgba(255,255,255,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid rgba(255,255,255,0.15)' }}>
                        {item.icon}
                      </span>
                      <div>
                        <div style={{ color: 'inherit', fontWeight: 500, fontSize: 14, fontFamily: "'Akshar', sans-serif" }}>{item.label}</div>
                        <div style={{ color: 'rgba(255,255,255,0.65)', fontSize: 12, marginTop: 2, fontFamily: "'Gotham', 'Helvetica Neue', Arial, sans-serif", fontWeight: 300 }}>{item.desc}</div>
                      </div>
                    </Link>
                  )
                })}
                <Link
                  href="/services"
                  onClick={closeMobile}
                  style={{ display: 'block', padding: '14px 5vw', color: 'rgba(255,255,255,0.82)', fontSize: 13.5, fontWeight: 500, fontFamily: "'Akshar', sans-serif", textDecoration: 'none', borderBottom: '1px solid rgba(255,255,255,0.12)', background: 'rgba(255,255,255,0.04)' }}
                >
                  View all Services →
                </Link>
              </div>
            ...
            >
              <span style={{ color: activeProducts ? C.p2 : '#FFFFFF', fontSize: 15, fontFamily: "'Akshar', sans-serif", fontWeight: activeProducts ? 600 : 500 }}>
                Products
              </span>
              <ChevronDown size={14} color="rgba(255,255,255,0.7)" style={{ transform: mobileExpanded === 'products' ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s' }} />
            </button>
            ...
            >
              <span style={{ color: activeAbout ? C.p2 : '#FFFFFF', fontSize: 15, fontFamily: "'Akshar', sans-serif", fontWeight: activeAbout ? 600 : 500 }}>
                About
              </span>
              <ChevronDown size={14} color="rgba(255,255,255,0.7)" style={{ transform: mobileExpanded === 'about' ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s' }} />
            </button>
